...
class SparseParallelSolverTest : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    SparseParallelSolver<MtxMatrix<double>, MtxVector<double>> sparseParallelSolver;
    double *correctAnswer, *sparseCorrectAnswer;
    int NZ_Sparse = 2;
};

TEST_F(SparseParallelSolverTest, test01_4x4_dense) {
    MtxVector<double> b;
    ...
    SolverResult sr = sparseParallelSolver.solve(&m, &b);
    b.save("./", "_[dense]_sparse_parallel.mtx");
    for (int i = 0; i < 4; i++) {
...
TEST_F(SparseParallelSolverTest, test01_4x4_sparse) {
    MtxVector<double> b;
    ...
    SolverResult sr = sparseParallelSolver.solve(&m, &b);
    b.save("./", "_[sparse]_sparse_parallel.mtx");
